...
// Validating the Text of Final Value added to Cart :  
    const finalAddToCartPrice = await $("//span[@class='cart-recap__price-line-price']");//Xpath of £10
    const kk = await finalAddToCartPrice.getText(); // This will get text of £10
    console.log("String Total Product Price is : - "+ kk);
    const productFinalIntPrice =  kk.replace('£', ''); // This will replace '£' with No Space (Earlier I tried Trim Method it was giving an error)
    console.log("The Total Product Price is :- " + productFinalIntPrice); 
    let one =  await Math.floor(productFinalIntPrice); // This will convert String to Number 

...
class AddNoteReminder{
    ...
    async fillTheValueOfHour(){
        ...
        await (await this.clickOnHour).setValue(H.Hour);
    }
    ...
    async backPage(){
     ...
     async verifyRemionderTime(){
         const actualTimeText = await $('//android.widget.TextView[@resource-id="com.socialnmobile.dictapps.notepad.color.note:id/view_note"]')
         await expect(actualTimeText).toHaveText(name.Name);
